...
class BseFetcher:
    """
    Discovers and downloads investor documents from BSE.

    Discovery order:
      1. BSE Corporate Announcements API (primary)
      2. Curated fallback URLs (skipped for generic fetcher)

    Deduplication is enforced by DocumentTracker via UNIQUE(company_id, source_url).
    """

    def __init__(self, company_id: str, scrip_code: str):
        self.company_id = company_id
        self.scrip_code = scrip_code
        self.pdf_dir = Path(f"data/pdfs/{company_id}")
    ...
    def run(
        ...
        if bse_processed == 0:
            logger.warning("BSE API returned 0 relevant results.")
            # We don't use FALLBACK_DOCUMENTS here because it's hardcoded for Paytm.
            # Fallback removed for generic generic fetcher.
